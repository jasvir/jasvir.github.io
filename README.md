...
and JavaScript interactions were imported unchanged from `jasvir/jasvir`.
...
- Scene models, materials and motion: `dist/script.js`.
...
format and layout. This migration intentionally does not refactor the scene;
extracting the shared scene API is the next step.
...
```sh
node --test tests/*.test.mjs examples/island-profile-spike/verify.test.mjs
```
...
repository’s `dist/` directly. Its generated SVG is a compatibility fixture,
